...
async def get_indicators(symbol: str, tf: str = "1d", at: str = "latest"):
    ...
    if not row:
        raise HTTPException(404, "No indicators")
...
# ---------- Jobs (trigger analysis now) + Telegram test ----------
...
async def telegram_test():
    ok = _tg_send("✅ اختبار: تنبيهات الأسهم/الكريبتو تعمل.")
    return {"ok": ok, "note": "ضَع TELEGRAM_BOT_TOKEN و TELEGRAM_CHAT_ID في .env إن كان ok=false"}
